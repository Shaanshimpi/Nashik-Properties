...
const Home = () => {
  ...
  const searchPlaceholder = useMemo(() => 
    isMobile 
      ...
      : 'Search projects by location, type, or features...',
    [isMobile]
  );

  const isShowingFeatured = useMemo(() => 
    featuredProperties.some(prop => 
      prop._embedded?.['wp:term']?.[0]?.some(term => term.slug === 'featured')
    ),
    [featuredProperties]
  );

  // Memoized empty projects component
  const emptyProjectsComponent = useMemo(() => (
  ...
  const handleSearchSubmit = useCallback((e) => {
    ...
    if (trimmedQuery) {
      navigate(`/projects?search=${encodeURIComponent(trimmedQuery)}`);
    }
        ...
        <div className="container">
          <div className="section-header">
            ...
            <div className="section-header__actions">
              <Button href="/projects" onClick={navigateToProjects} variant="outline">
                View All Projects
              </Button>
            </div>
          </div>
          ...
          <ProjectList
            ...
            emptyMessage={emptyProjectsComponent}
            showLoadMore={false}
          />
        </div>
      </section>
